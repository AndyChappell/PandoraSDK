 ...
 *  shared file-identity state (name, type, active container). Also carries the
 *  SchemaRegistry and FileMetadata loaded from (or to be written to) the global
 *  header, making them available to both the reader and writer hierarchies.
 */
class Persistency
{
public:
...
inline const std::string &Persistency::GetFileName() const { return m_fileName; }
inline FileType Persistency::GetFileType() const           { return m_fileType; }
inline FileMetadata &Persistency::GetFileMetadata()        { return m_metadata; }
inline const SchemaRegistry &Persistency::GetSchemaRegistry() const { return m_schemaRegistry; }
